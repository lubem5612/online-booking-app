        ...
        <div v-else class="mt-4">
            ...
            >
                ...
                <template v-slot:[`item.action`]="{ item }">
                    <v-btn small color="primary pa-2" @click.prevent="checkinBook(item)">check-in book</v-btn>
                </template>
            </v-data-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            resetSearch() {
                this.GET_BOOKS({search: '', date: '', page: 1, status: 'checked_out'});
            },
            getBooks() {
            ...
            checkinBook(item) {
                ...
                api.post(`/api/bookings/${item.id}`, data).then(res => {
                    Toast.fire({icon: 'success', title: 'booking checked in successful'});
                    this.getBooks();
                }).catch(err => {
